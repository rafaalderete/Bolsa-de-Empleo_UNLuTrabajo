...
@section('bodyIndice')
  ...
  <div class="row">
    <div id="breadcrumb" class="col-xs-12">
      <ol class="breadcrumb">
        <li><a>Reportes</a></li>
        <li><a>Detalle Empresas con más Propuestas para mi Carrera</a></li>
      </ol>
    </div>
  </div>
...
<div class="row" style="margin-top:-20px;">
  ...
  <div class="box no-box-shadow">
    ...
    <div class="box-content dropbox" style="width:70%; margin: 0 auto;">
      <h4 class="page-header">Reporte - Detalle Empresas con más Propuestas para mi Carrera
        @if(count($empConPropParaMiCarrera) > 0)
          @if(true)
            <a href="{{route('in.reportes.estudiante.tablaspdf.empresas-propuestas-mi-carrera')}}"  style="margin-top: -5px" class="btn btn-info pull-right btn-registrar-3">
      ...
      @if(count($empConPropParaMiCarrera) > 0)
        ...
        <table class="table table-bordered table-striped table-hover table-heading table-datatable" id="dev-table">
          ...
          <tbody>
            @foreach( $empConPropParaMiCarrera as $key => $empresaPropuestaCarrera )
              <tr>
                <td>{{$key + 1}}</td>
                <td>{{$empresaPropuestaCarrera->nombre_comercial}}</td>
                <td>{{$empresaPropuestaCarrera->cantidad}}</td>
              </tr>
            @endforeach
          </tbody>
        </table>
