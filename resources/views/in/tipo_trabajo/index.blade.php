    ...
    <div class="box" style="margin-top: -20px">
      ...
      <div class="box-content dropbox">
        ...
        <table class="table table-bordered table-striped table-hover table-heading table-datatable" id="dev-table">
          ...
          <tbody>
            @foreach( $tipo_trabajo as $ttrabajo)
              <tr>
                <td>{{ $ttrabajo->id }}</td>
                <td>{{ $ttrabajo->nombre_tipo_trabajo }}</td>
                <td>{{ $ttrabajo->estado }}</td>
                <!-- envio el parametro del metodo edit y destroy-->
                <td>
                  @if(Entrust::can('modificar_tipo_trabajo'))
                    <a href="{{ route('in.tipo_trabajo.edit', $ttrabajo->id) }}" class="btn btn-primary"><span class="fa fa-pencil" aria-hidden="true"></span></a>
                  @endif
                  @if(Entrust::can('eliminar_tipo_trabajo'))
                    {!! Form::open(['route' => ['in.tipo_trabajo.destroy', $ttrabajo->id], 'method' => 'DELETE', 'style' => "display: inline-block"]) !!}
                    <a href="" class="btn btn-danger" data-toggle="modal" data-target="#delSpk" data-title="Eliminar Tipo Trabajo"
                      data-message="¿Seguro que quiere eliminar el Tipo Trabajo {{$ttrabajo->nombre_tipo_trabajo}}?"><span class=" fa fa-trash-o" aria-hidden="true"></span></a>
                    {!! Form::close() !!}
                  @endif
                  </td>
                </tr>
